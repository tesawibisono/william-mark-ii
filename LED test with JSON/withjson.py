...
def load_led_pins(json_file):
    with open(json_file, 'r') as file:
        led_pins = json.load(file)
    return led_pins

def main():
    
    led_pins = load_led_pins('led_pins.json')

    while True:
        user_input = input("Tell me to do something: ")
        gpt_prompt = f"red LED on pin {led_pins['leds']['red']}, yellow LED on pin {led_pins['leds']['yellow']}, and white LED on pin {led_pins['leds']['white']}. Generate Python code to control Raspberry Pi 5 using gpiod as the gpio library with gpiod.Chip('gpiochip4') and add other necessary library for this: {user_input}. ONLY GENERATE THE CODE. DON'T EXPLAIN WITHOUT ''#''"
